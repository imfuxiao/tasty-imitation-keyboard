...
class HostingAppViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    NotificationCenter.default.addObserver(
      self, selector: #selector(HostingAppViewController.keyboardWillShow),
      name: UIResponder.keyboardWillShowNotification, object: nil)
    NotificationCenter.default.addObserver(
      self, selector: #selector(HostingAppViewController.keyboardDidHide),
      name: UIResponder.keyboardDidHideNotification, object: nil)
    // NSNotificationCenter.defaultCenter().addObserver(self, selector: Selector("keyboardWillChangeFrame:"), name: UIKeyboardWillChangeFrameNotification, object: nil)
    NotificationCenter.default.addObserver(
      self, selector: #selector(HostingAppViewController.keyboardDidChangeFrame(_:)),
  ...
  @objc func keyboardDidHide() {
    ...
    stats?.text = "(Waiting for keyboard...)"
  }
    ...
    if frameEnd.height == referenceHeight {
      if firstHeightTime == nil {
        ...
        if let startTime = startTime {
          if let firstHeightTime = firstHeightTime {
            let formatString = NSString(
              format: "First: %.2f, Total: %.2f", firstHeightTime - startTime,
              firstHeightTime - startTime)
            stats?.text = formatString as String
          }
        }
      }
      ...
      if secondHeightTime == nil {
        ...
        if let startTime = startTime {
          if let firstHeightTime = firstHeightTime {
            if let secondHeightTime = secondHeightTime {
              let formatString = NSString(
                format: "First: %.2f, Second: %.2f, Total: %.2f", firstHeightTime - startTime,
                secondHeightTime - firstHeightTime, secondHeightTime - startTime)
              stats?.text = formatString as String
            }
          }
        }
