...
class KeyboardViewController: UIInputViewController {
  ...
  var backspaceActive: Bool {
    return (self.backspaceDelayTimer != nil) || (self.backspaceRepeatTimer != nil)
  }
  ...
  override init(nibName nibNameOrNil: String?, bundle nibBundleOrNil: Bundle?) {
    ...
    NotificationCenter.default.addObserver(
      self, selector: #selector(KeyboardViewController.defaultsChanged(_:)),
  ...
  func setupKludge() {
    if self.kludge == nil {
      ...
      let a = NSLayoutConstraint(
        item: kludge, attribute: NSLayoutConstraint.Attribute.left,
      ...
      let b = NSLayoutConstraint(
        item: kludge, attribute: NSLayoutConstraint.Attribute.right,
        relatedBy: NSLayoutConstraint.Relation.equal, toItem: self.view,
        attribute: NSLayoutConstraint.Attribute.left, multiplier: 1, constant: 0)
      let c = NSLayoutConstraint(
        item: kludge, attribute: NSLayoutConstraint.Attribute.top,
        relatedBy: NSLayoutConstraint.Relation.equal, toItem: self.view,
        attribute: NSLayoutConstraint.Attribute.top, multiplier: 1, constant: 0)
      let d = NSLayoutConstraint(
        item: kludge, attribute: NSLayoutConstraint.Attribute.bottom,
        relatedBy: NSLayoutConstraint.Relation.equal, toItem: self.view,
        attribute: NSLayoutConstraint.Attribute.top, multiplier: 1, constant: 0)
  ...
  func setupLayout() {
    if !self.constraintsAdded {
      self.layout = type(of: self).layoutClass.init(
        model: self.keyboard, superview: self.forwardingView,
        layoutConstants: type(of: self).layoutConstants, globalColors: type(of: self).globalColors,
        darkMode: self.darkMode(), solidColorMode: self.solidColorMode())
  ...
  override func viewDidLayoutSubviews() {
    ...
    let orientationSavvyBounds = CGRect(
      x: 0, y: 0, width: self.view.bounds.width,
      height: self.height(forOrientation: self.interfaceOrientation, withTopBanner: false))

    if self.lastLayoutBounds != nil && self.lastLayoutBounds == orientationSavvyBounds {
      // do nothing
    } else {
      let uppercase = self.shiftState.uppercase()
      let characterUppercase =
        (UserDefaults.standard.bool(forKey: kSmallLowercase) ? uppercase : true)

      self.forwardingView.frame = orientationSavvyBounds
      self.layout?.layoutKeys(
        self.currentMode, uppercase: uppercase, characterUppercase: characterUppercase,
    ...
    self.bannerView?.frame = CGRect(
      x: 0, y: 0, width: self.view.bounds.width, height: metric("topBanner"))
  ...
  override func viewWillAppear(_ animated: Bool) {
    ...
    self.keyboardHeight = self.height(
      forOrientation: self.interfaceOrientation, withTopBanner: true)
  }
  ...
  func height(forOrientation orientation: UIInterfaceOrientation, withTopBanner: Bool) -> CGFloat {
    ...
    return CGFloat(
      orientation.isPortrait
        ? canonicalPortraitHeight + topBannerHeight : canonicalLandscapeHeight + topBannerHeight)
  }
  ...
  func setupKeys() {
    ...
    for page in self.keyboard.pages {
          ...
          if let keyView = self.layout?.viewForKey(key) {
            ...
            case Key.KeyType.backspace:
              let cancelEvents: UIControl.Event = [
                UIControl.Event.touchUpInside, UIControl.Event.touchUpInside,
                UIControl.Event.touchDragExit, UIControl.Event.touchUpOutside,
                UIControl.Event.touchCancel, UIControl.Event.touchDragOutside,
              ]
            ...
            if key.type != Key.KeyType.shift && key.type != Key.KeyType.modeChange {
              ...
              keyView.addTarget(
                ...
                for: [
                  .touchUpInside, .touchUpOutside, .touchDragOutside, .touchDragExit, .touchCancel,
                ])
            }
  ...
  @objc func hidePopupDelay(_ sender: KeyboardKey) {
    ...
    if sender.popup != nil {
      self.popupDelayTimer = Timer.scheduledTimer(
        timeInterval: 0.05, target: self,
        selector: #selector(KeyboardViewController.hidePopupCallback), userInfo: nil, repeats: false
      )
    }
  }
  ...
  func setHeight(_ height: CGFloat) {
    if self.heightConstraint == nil {
      self.heightConstraint = NSLayoutConstraint(
        ...
        multiplier: 0,
        constant: height)
      self.heightConstraint!.priority = .init(rawValue: 1000)
  ...
  @objc func backspaceDown(_ sender: KeyboardKey) {
    ...
    self.backspaceDelayTimer = Timer.scheduledTimer(
      timeInterval: self.backspaceDelay - self.backspaceRepeat, target: self,
      selector: #selector(KeyboardViewController.backspaceDelayCallback), userInfo: nil,
      repeats: false)
  }
  ...
  @objc func backspaceDelayCallback() {
    self.backspaceDelayTimer = nil
    self.backspaceRepeatTimer = Timer.scheduledTimer(
      timeInterval: self.backspaceRepeat, target: self,
      selector: #selector(KeyboardViewController.backspaceRepeatCallback), userInfo: nil,
      repeats: true)
  }
  ...
  @objc func updateKeyCaps(_ uppercase: Bool) {
    let characterUppercase =
      (UserDefaults.standard.bool(forKey: kSmallLowercase) ? uppercase : true)
    self.layout?.updateKeyCaps(
      false, uppercase: uppercase, characterUppercase: characterUppercase,
      shiftState: self.shiftState)
  }
  ...
  func setMode(_ mode: Int) {
    ...
    let characterUppercase =
      (UserDefaults.standard.bool(forKey: kSmallLowercase) ? uppercase : true)
    self.layout?.layoutKeys(
      mode, uppercase: uppercase, characterUppercase: characterUppercase,
      shiftState: self.shiftState)
    ...
    if self.settingsView == nil {
      if let aSettings = self.createSettings() {
        ...
        let widthConstraint = NSLayoutConstraint(
          item: aSettings, attribute: NSLayoutConstraint.Attribute.width,
          relatedBy: NSLayoutConstraint.Relation.equal, toItem: self.view,
          attribute: NSLayoutConstraint.Attribute.width, multiplier: 1, constant: 0)
        let heightConstraint = NSLayoutConstraint(
          item: aSettings, attribute: NSLayoutConstraint.Attribute.height,
          relatedBy: NSLayoutConstraint.Relation.equal, toItem: self.view,
          attribute: NSLayoutConstraint.Attribute.height, multiplier: 1, constant: 0)
        let centerXConstraint = NSLayoutConstraint(
          item: aSettings, attribute: NSLayoutConstraint.Attribute.centerX,
          relatedBy: NSLayoutConstraint.Relation.equal, toItem: self.view,
          attribute: NSLayoutConstraint.Attribute.centerX, multiplier: 1, constant: 0)
        let centerYConstraint = NSLayoutConstraint(
          item: aSettings, attribute: NSLayoutConstraint.Attribute.centerY,
          relatedBy: NSLayoutConstraint.Relation.equal, toItem: self.view,
  ...
  func stringIsWhitespace(_ string: String?) -> Bool {
    if let string = string {
      for char in string {
        if !self.characterIsWhitespace(char) {
          return false
        }
      }
    }
      ...
      case .sentences:
        if let beforeContext = documentProxy.documentContextBeforeInput {
          ...
          for i in 0..<offset {
            ...
            if self.characterIsPunctuation(char) {
              if i == 0 {
              ...
              } else if self.characterIsNewline(char) {
  ...
  class var layoutClass: KeyboardLayout.Type { return KeyboardLayout.self }
  class var layoutConstants: LayoutConstants.Type { return LayoutConstants.self }
  class var globalColors: GlobalColors.Type { return GlobalColors.self }
  ...
  func createSettings() -> ExtraView? {
    // note that dark mode is not yet valid here, so we just put false for clarity
    let settingsView = DefaultSettings(
      globalColors: type(of: self).globalColors, darkMode: false,
      solidColorMode: self.solidColorMode())
    settingsView.backButton?.addTarget(
      self, action: #selector(KeyboardViewController.toggleSettings),
