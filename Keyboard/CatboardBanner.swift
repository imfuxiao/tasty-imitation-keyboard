...
class CatboardBanner: ExtraView {
  var catSwitch: UISwitch = .init()
  var catLabel: UILabel = .init()
  ...
  required init(globalColors: GlobalColors.Type?, darkMode: Bool, solidColorMode: Bool) {
    ...
    self.catSwitch.addTarget(
      self, action: #selector(CatboardBanner.respondToSwitch), for: UIControl.Event.valueChanged)
  ...
  override func layoutSubviews() {
    ...
    self.catLabel.frame.origin = CGPoint(
      x: self.catSwitch.frame.origin.x + self.catSwitch.frame.width + 8,
      y: self.catLabel.frame.origin.y)
  }
