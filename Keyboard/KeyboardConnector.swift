...
class KeyboardConnector: KeyboardKeyBackground {
  ...
  init(
    cornerRadius: CGFloat, underOffset: CGFloat, start s: UIView, end e: UIView,
    startConnectable sC: Connectable, endConnectable eC: Connectable, startDirection: Direction,
    ...
    self.startConnectable = sC
    self.endConnectable = eC
  ...
  func resizeFrame() {
    self.generateConvertedPoints()

    let buffer: CGFloat = 32
    self.offset = CGPoint(x: buffer / 2, y: buffer / 2)

    let minX = min(
      convertedStartPoints.0.x, self.convertedStartPoints.1.x, self.convertedEndPoints.0.x,
      self.convertedEndPoints.1.x)
    let minY = min(
      convertedStartPoints.0.y, self.convertedStartPoints.1.y, self.convertedEndPoints.0.y,
      self.convertedEndPoints.1.y)
    let maxX = max(
      convertedStartPoints.0.x, self.convertedStartPoints.1.x, self.convertedEndPoints.0.x,
      self.convertedEndPoints.1.x)
    let maxY = max(
    ...
    self.frame = CGRect(
      x: minX - buffer / 2, y: minY - buffer / 2, width: width + buffer, height: height + buffer)
  }
  ...
  override func generatePointsForDrawing(_ bounds: CGRect) {
    ...
    let isVertical =
      (self.startDir == Direction.up || self.startDir == Direction.down)
      && (self.endDir == Direction.up || self.endDir == Direction.down)
