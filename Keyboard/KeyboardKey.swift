...
class KeyboardKey: UIControl {
  ...
  var text: String {
    didSet {
      ...
      self.label.frame = CGRect(
        x: self.labelInset, y: self.labelInset, width: self.bounds.width - self.labelInset * 2,
        height: self.bounds.height - self.labelInset * 2)
      self.redrawText()
    }
  }

  var color: UIColor { didSet { self.updateColors() } }
  ...
  func refreshShapes() {
    ...
    // TODO: withUnder
    let addCurves = {
      (fromShape: KeyboardKeyBackground?, toPath: UIBezierPath, toEdgePaths: UIBezierPath) in
      if let shape = fromShape {
        let path = shape.fillPath
        let translatedUnitSquare = self.displayView.convert(unitSquare, from: shape)
        let transformFromShapeToView = CGAffineTransform(
          translationX: translatedUnitSquare.origin.x, y: translatedUnitSquare.origin.y)
  ...
  func redrawShape() {
    if let shape = self.shape {
      ...
      let size = CGSize(
        width: self.bounds.width - pointOffset - pointOffset,
        height: self.bounds.height - pointOffset - pointOffset)
      shape.frame = CGRect(
  ...
  func configurePopup(_ direction: Direction) {
    ...
    self.connector = KeyboardConnector(
      cornerRadius: 4, underOffset: self.underOffset, start: kv, end: p, startConnectable: kv,
      endConnectable: p, startDirection: direction, endDirection: direction.opposite())
